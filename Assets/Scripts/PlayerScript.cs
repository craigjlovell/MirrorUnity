...
namespace QuickStart
{
    ...
    {        
        ...
        void OnWeaponChanged(int a_old, int a_new)
        {
            // disable old weapon
            // in range and not null
            if (0 < a_old && a_old < weaponArray.Length && weaponArray[a_old] != null)
                weaponArray[a_old].SetActive(false);

            // enable new weapon
            // in range and not null
            if (0 < a_new && a_new < weaponArray.Length && weaponArray[a_new] != null)
            {
                weaponArray[a_new].SetActive(true);
        ...
        void OnColorChanged(Color a_old, Color a_new)
        {
            playerNameText.color = a_new;
            playerMaterialClone = new Material(GetComponent<Renderer>().material);
            playerMaterialClone.color = a_new;
            GetComponent<Renderer>().material = playerMaterialClone;
        }

        [Command]
        public void CmdSetupPlayer(string a_name, Color a_col)
        {
